...
bool paralution_init_ocl(void) {
  ...
  if (ocl_numPlatforms < 1) {
    ...
    err  = clBuildProgram( OCL_HANDLE(_Backend_Descriptor.OCL_handle)->OCL_program_sp,  0, NULL, "-w -Werror -cl-mad-enable -cl-no-signed-zeros", NULL, NULL );
    err |= clBuildProgram( OCL_HANDLE(_Backend_Descriptor.OCL_handle)->OCL_program_dp,  0, NULL, "-w -Werror -cl-mad-enable -cl-no-signed-zeros", NULL, NULL );
    err |= clBuildProgram( OCL_HANDLE(_Backend_Descriptor.OCL_handle)->OCL_program_int, 0, NULL, "-w -Werror -cl-mad-enable -cl-no-signed-zeros", NULL, NULL );
