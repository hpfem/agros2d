...
# normal elements
error_data = np.loadtxt("error-normal_elements.dat")
steps_e = error_data[:, 0]
error_e = error_data[:, 1]
dofs_data = np.loadtxt("dofs-normal_elements.dat")
dofs_e = dofs_data[:, 1]
...
pl.figure()
pl.plot(dofs_e, error_e, '.-', label="$\mathrm{normal~elements}$")
...
pl.figure()
pl.plot(steps_e, error_e, '.-', label="$\mathrm{normal~elements}$")
...
pl.figure()
pl.plot(steps_e, dofs_e, '.-', label="$\mathrm{normal~elements}$")
