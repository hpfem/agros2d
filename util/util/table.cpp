...
double Table::get_A(unsigned int i, unsigned int j)
{
    if ((i >= this->size) || (j >= this->size))
        throw AgrosException("Invalid access to a Butcher's table.");
    return this->A[i][j];
}
...
ButcherTable::ButcherTable(ButcherTableType butcher_table)
{
    ...
    default: throw AgrosException("Unknown Butcher's table.");
    }
}
...
{
    if (i > size) throw AgrosException("Invalid access to a Butcher's table.");
    return this->B[i];
}
...
bool ButcherTable::is_explicit()
{
    ...
    for (unsigned int i = 0; i < size; i++)
    {
        for (unsigned int j = 0; j<size; j++)
        {
            ...
            if (j >= i && fabs(val_ij) > EPS_ZERO) result = false;
        }
    }
...
bool ButcherTable::is_fully_implicit()
{
    ...
    for (unsigned int i = 0; i < size; i++)
    {
        ...
        {
            double val_ij = get_A(i, j);
            if (j > i && fabs(val_ij) > EPS_ZERO) result = true;
        }
    }
...
void ButcherTable::switch_B_rows()
{
    ...
    if (this->is_embedded() == false)
        throw AgrosException("ButcherTable::switch_B_rows(): Zero B2 row detected.");
