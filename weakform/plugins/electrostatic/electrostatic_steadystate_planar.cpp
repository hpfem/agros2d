...
using namespace electrostaticsteadystateplanar;
                                          ...
                                          Hermes::Hermes2D::Func<double> *v, Hermes::Hermes2D::Geom<double> *e, Hermes::Hermes2D::ExtData<Scalar> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
       result += wt[i] * (el_epsr.number()*EPS0*(u->dx[i]*v->dx[i]+u->dy[i]*v->dy[i]));
    }
    return result;
}
                                             ...
                                             Hermes::Hermes2D::Func<Hermes::Ord> *v, Hermes::Hermes2D::Geom<Hermes::Ord> *e, Hermes::Hermes2D::ExtData<Hermes::Ord> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
       result += wt[i] * (1*EPS0*(u->dx[i]*v->dx[i]+u->dy[i]*v->dy[i]));
    }	
    return result;
}
                                          ...
                                          Hermes::Hermes2D::Geom<double> *e, Hermes::Hermes2D::ExtData<Scalar> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
        result += wt[i] * (el_rho.number()*v->val[i]);
    }
    return result;
}
                                          ...
                                          Hermes::Hermes2D::Geom<double> *e, Hermes::Hermes2D::ExtData<Scalar> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
        result += wt[i] * (el_epsr.number()*EPS0*(u_ext[this->j]->dx[i]*v->dx[i]+u_ext[this->j]->dy[i]*v->dy[i])-el_rho.number()*v->val[i]);
    }
    return result;
}
                                             ...
                                             Hermes::Hermes2D::Geom<Hermes::Ord> *e, Hermes::Hermes2D::ExtData<Hermes::Ord> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
       result += wt[i] * (1*EPS0*(u_ext[this->j]->dx[i]*v->dx[i]+u_ext[this->j]->dy[i]*v->dy[i])-1*v->val[i]);
    }	
    return result;
}
                                           ...
                                           Hermes::Hermes2D::Geom<double> *e, Hermes::Hermes2D::ExtData<Scalar> *ext) const
{
    ...
    for (int i = 0; i < n; i++)
    {
        result += wt[i] * (sigma.number()*v->val[i]);
    }
    return result;
}
