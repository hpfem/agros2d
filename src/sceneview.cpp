...
void SceneView::resizeGL(int w, int h)
{
    ...
    if (Util::problem()->isSolved() && m_sceneMode == SceneMode_Postprocessor)
    {
        paletteFilter();
...
void SceneView::paintGL()
{
    ...
    if (is3DMode())
    {
        if (Util::problem()->isMeshed() && (m_sceneMode == SceneMode_Postprocessor))
        {
...
void SceneView::paintRulersHints()
{
    logMessage("SceneView::paintRulersHints()");
    ...
    if (m_snapToGrid)
    {
        ...
        renderTextPos(snapPoint.x + m_rulersAreaWidth.x, snapPoint.y - m_rulersAreaWidth.y, 0.0,
...
void SceneView::paintSceneModeLabel()
{
    ...
    case SceneMode_Postprocessor:
        ...
        case SceneViewPostprocessorShow_ScalarView3DSolid:
            text = QString::fromStdString(m_sceneViewSettings.scalarPhysicFieldVariable != "" ? Util::scene()->activeViewField()->module()->get_variable(m_sceneViewSettings.scalarPhysicFieldVariable)->name : "");
...
void SceneView::doInvalidated()
{
    ...
    actSceneModePostprocessor->setEnabled(Util::problem()->isSolved());
    actSceneViewSelectMarker->setEnabled(Util::problem()->isSolved());
...
void SceneView::renderTextPos(double x, double y, double z,
                              const QString &str, bool blend, QFont fnt, bool horizontal)
{
    ...
    if (horizontal)
        renderText(x, y, z, str, fontLocal);
    else
        for (int i = 0; i < str.length(); i++)
            renderText(x, y - i*size.y, z, str[i], fontLocal);
}
