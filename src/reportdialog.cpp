...
void ReportDialog::doShowReport()
{
    ...
    QString styleSheetFilename = QString("%1/report/default.css").arg(datadir() + TEMPLATEROOT);

    bool fileStyleOK = QFile::copy(styleSheetFilename,
...
{
    QString templateFilename = QString("%1/report/default.tpl").arg(datadir() + TEMPLATEROOT);
    ...
    if (fileIndex.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        stream << replaceTemplates(templateFilename);
