...
namespace Hermes
{
  namespace Hermes2D
  {
    ...
    double NewtonSolverAgros<Scalar>::max_allowed_residual_norm = 1e12;

    template<typename Scalar>
    NewtonSolverAgros<Scalar>::NewtonSolverAgros(DiscreteProblem<Scalar>* dp) : NonlinearSolver<Scalar>(dp), kept_jacobian(NULL)
    {
      init_linear_solver();
    }

    template<typename Scalar>
    NewtonSolverAgros<Scalar>::NewtonSolverAgros(DiscreteProblem<Scalar>* dp, Hermes::MatrixSolverType matrix_solver_type) : NonlinearSolver<Scalar>(dp, matrix_solver_type), kept_jacobian(NULL)
    {
      init_linear_solver();
    }

    template<typename Scalar>
    void NewtonSolverAgros<Scalar>::init_linear_solver()
    {
    ...
    template<typename Scalar>
    NewtonSolverAgros<Scalar>::~NewtonSolverAgros()
    {
    ...
    template<typename Scalar>
    bool NewtonSolverAgros<Scalar>::solve(Scalar* coeff_vec)
    {
      return solve(coeff_vec, 1E-8, 100, false);
    }

    template<typename Scalar>
    bool NewtonSolverAgros<Scalar>::solve(Scalar* coeff_vec, bool residual_as_function)
    {
      return solve(coeff_vec, 1E-8, 100, residual_as_function);
    }

    template<typename Scalar>
    bool NewtonSolverAgros<Scalar>::solve(Scalar* coeff_vec, double newton_tol, int newton_max_iter, bool residual_as_function)
    {      
    ...
    template<typename Scalar>
    bool NewtonSolverAgros<Scalar>::solve_keep_jacobian(Scalar* coeff_vec, bool residual_as_function)
    {
      return solve(coeff_vec, 1E-8, 100, residual_as_function);
    }

    template<typename Scalar>
    bool NewtonSolverAgros<Scalar>::solve_keep_jacobian(Scalar* coeff_vec, double newton_tol, int newton_max_iter, bool residual_as_function)
    {
      // Obtain the number of degrees of freedom.
      int ndof = this->dp->get_num_dofs();
    ...
    template<typename Scalar>
    void NewtonSolverAgros<Scalar>::set_iterative_method(const char* iterative_method_name)
    {
    ...
    template<typename Scalar>
    void NewtonSolverAgros<Scalar>::set_preconditioner(const char* preconditioner_name)
    {
    ...
    template class HERMES_API NewtonSolverAgros<double>;
    template class HERMES_API NewtonSolverAgros<std::complex<double> >;
  }
}
