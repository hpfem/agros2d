...
void LocalPointValue::calculate()
{
    ...
    if (Util::scene()->sceneSolution()->isSolved())
    {
        ...
        if (index != -1)
        {
            ...
            SceneMaterial *tmpMaterial = Util::scene()->labels->at(atoi(Util::scene()->sceneSolution()->meshInitial()->get_element_markers_conversion().get_user_marker(e->marker).marker.c_str()))->marker;
